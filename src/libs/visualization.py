...
def smallMultiplesForecastV2(historicDf, forecastDf, historicPeriod, forecastPeriod, targetRegions, outputDir, outputName, maPeriod=None):
    ...
    for region in targetRegions:
        ...
        if maPeriod is not None:
            historicDf.loc[region].loc[historicPeriod[0]:historicPeriod[-1]]['new-cases'].rolling(maPeriod, min_periods=1).mean().plot(marker='', color='grey', linewidth=2.4, alpha=0.9, ax=plt.gca())
        else:
            historicDf.loc[region].loc[historicPeriod[0]:historicPeriod[-1]]['new-cases'].plot(marker='', color='grey', linewidth=2.4, alpha=0.9, ax=plt.gca())

        forecastDf.loc[slice(forecastPeriod[0], forecastPeriod[-1])].xs(region, level=1)['predictedCases'].plot(marker='', color='black', linewidth=2.4, alpha=0.9,   ax=plt.gca())
...
def smallMultiplesGenericWithProjected(dataPerRegion, dataPerRegion2, targetRegions, attributes, outputDir, outputName, prints=True):
    plotsPerRow = 5
    numPlots = len(targetRegions)
    numRows = math.ceil((numPlots / plotsPerRow))
    ...
    for region in targetRegions:
        ...
        dataToPlot = [dataPerRegion[region][att] for att in attributes]
        dataToPlot2 = [dataPerRegion2[region][att] for att in attributes]
        indexes = [[i for i in range(len(t))] for t in dataToPlot]
